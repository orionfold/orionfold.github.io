...
assert.match(data, /label: 'App payload'[\s\S]*value: 'Models download separately'/, 'current specifications do not reuse a historical installer size');
assert.doesNotMatch(data, /Around 40 MB|19\.2 MiB of the app payload/, 'historical payload measurements must not masquerade as the v1.6 installer');
...
for (const provider of ['Flow Runtime', 'Ollama', 'Anthropic', 'OpenAI', 'OpenRouter', 'LM Studio']) {
...
assert.match(pricing, /58 documents?[\s\S]*24 assets/, 'Tech Specs uses the current Flow Guide corpus size');
...
assert.match(read('src/components/Footer.astro'), /href: '\/arena\/', label: 'Arena'/, 'Arena remains in the shared Orionfold footer');
