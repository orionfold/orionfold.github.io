...
assert.match(home, /Race control · For enterprise AI owners/, 'enterprise proof remains race control');
assert.match(home, /Final lap · The origin story/, 'origin story remains the final lap');
...
assert.match(hero, /flow-launch-hero__product[\s\S]*<FlowShot[\s\S]*src=\{shotExpandHoverPanel\}/, 'right side is the selected real Flow product proof');
...
assert.match(pitStop, /Flow Capabilities \. Storyboard/, 'high-fidelity creative is framed as the Flow capabilities storyboard');
...
assert.match(home, /id="home-race-control"[\s\S]*aria-labelledby="home-race-control-title"/, 'race control exposes the fourth storyboard target');
